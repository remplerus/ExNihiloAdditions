...
public class EXAHammerLootModifier extends LootModifier {
    public static final Supplier<Codec<EXAHammerLootModifier>> CODEC = Suppliers.memoize(() ->
            RecordCodecBuilder.create(instance -> codecStart(instance)
                    .apply(instance, EXAHammerLootModifier::new)));

    public EXAHammerLootModifier(LootItemCondition[] conditionsIn) {
        super(conditionsIn);
    }
    ...
    protected @NotNull ObjectArrayList<ItemStack> doApply(ObjectArrayList<ItemStack> objectArrayList, LootContext lootContext) {
        ...
        if (tool != null && blockState != null && entity instanceof Player && tool.getItem() instanceof ItemModularHandheld &&
                tool.canPerformAction(TetraToolActions.hammer) && NihiloHelper.canHammer(blockState)) {
            ObjectArrayList<ItemStack> changedLoot = NihiloHelper.getHammerDrops(entity.level(), blockState, lootContext.getRandom());
