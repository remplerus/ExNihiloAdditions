...
public class EXAJsonParser {
    ...
    public SoundType getSoundType() {
        if (json.has("sound_type")) {
            String soundTypeName = json.get("sound_type").getAsString();
            SoundType soundType = SoundTypeResolver.VANILLA_SOUND_TYPES.get(soundTypeName);
            if (soundType == null) {
                error = true;
                ExNihiloAdditions.LOGGER.error("Invalid sound type: \"{}\" for item {}", soundTypeName, jsonPath);
            } else {
                return soundType;
            }
        } else {
            ExNihiloAdditions.LOGGER.error("Missing sound type for item {}", jsonPath);
            error = true;
            return SoundType.WOOD; // Default value
        }
        return SoundType.WOOD; // Default value
    }
    ...
    public float getStrength() {
        if (json.has("strength")) {
            try {
                return json.get("strength").getAsFloat();
            } catch (NumberFormatException e) {
                ExNihiloAdditions.LOGGER.error("Failed to add strength value for item {} with {}", jsonPath, json.get("strength").getAsString());
                error = true;
            }
        } else {
            if (this.registry == DefaultItems.SIEVES) {
                return 2.0F; // Default value for sieves
            }
            ExNihiloAdditions.LOGGER.error("Missing strength for item {}. Defaulting to 0F", jsonPath);
            error = true;
        }
    ...
    public boolean getOptionalBoolean(String key) {
        return json.get(key) instanceof JsonPrimitive primitive && primitive.isBoolean() && primitive.getAsBoolean();
    }

    public String getRequiredModID() {
        if (json.has("required_mod_id")) {
            String modId = json.get("required_mod_id").getAsString();
            if (modId.isEmpty()) {
                ExNihiloAdditions.LOGGER.error("Empty required mod ID for item {}", jsonPath);
                error = true;
            }
            return modId;
        } else {
    ...
    static class SoundTypeResolver {
